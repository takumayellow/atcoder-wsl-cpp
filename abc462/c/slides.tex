...
%====================================================================
\begin{frame}{これは「パレート最適」— ただし\underline{向き}に注意}
  \begin{block}{答え $=$ \textbf{最小化方向}のパレート最適（$=$ 左下フロンティア）}
    「自分より真の左下に点が無い」点 $=$ $X,Y$ を\textbf{同時に最小化}する
    意味でのパレート最適点．prefix 最小はこれを数えているので，
    \textcolor{ok}{「パレート最適」という捉え方は正しい（だから AC）}．
    \\[0.2em]
    双対：$X{\leftrightarrow}Y$ を入れ替え（$Y$ 昇順 $+$ $X$ prefix 最小）でも\textbf{同じ答え}．
  \end{block}

  \begin{alertblock}{落とし穴：\textbf{右上（最大化方向）}に取ると別物}
    覆う条件を $X_i<X_j \wedge Y_i<Y_j$（自分より\textbf{右上}に点）と書くと，
    数えるのは「右上に点が無い点 $=$ パレート\textbf{最大}点」になり\textbf{別の答え}．
  \end{alertblock}

  \footnotesize 反例：$(1,1),(2,3),(3,2)$ で\emph{正答 $=1$} だが右上版は $2$．
  公式サンプル $3$ でも\emph{正答 $=2$} に対し右上版は $4$ を返して WA
  （$N=5$ 全 $120$ 順列中 $74$ 通りでズレる）．
  \normalsize

  \vspace{0.3em}
  \textbf{要点}：効くのは「\textbf{左下}（最小化）」のパレートのみ．
