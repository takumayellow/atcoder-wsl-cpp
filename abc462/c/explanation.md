...
## 7. 正しい「双対」と、手書きメモ(memo.pdf)の落とし穴

### 正しい双対(答えは変わらない)

「真の左下に点が無いか」の判定は、**X と Y の役割を入れ替えても**同じ。
つまり **Y 昇順ソート + X の prefix 最小** でも**同じ答え**になる。
これは左下フロンティア(下側の階段)を、横から見るか縦から見るかの違いにすぎない。

### 落とし穴: 「右上に点があるか」は別問題

手書きメモには、覆う条件を **Xᵢ < Xⱼ ∧ Yᵢ < Yⱼ**(自分より**右上**に点がある)
と書いた箇所がある。これは向きを取り違えており、実際には
「右上に点が無い点(= Pareto 最大点)の数」を数えることになって**別の答え**を返す。

反例: 点 (1,1), (2,3), (3,2) のとき
- 正答(真の左下が無い点 = 左下フロンティア)は **1**((1,1) のみ)
- 右上版だと **2** を返してしまう

N=5 の全 120 順列のうち **74 通り**で両者の答えがズレることを全探索で確認済み。
この問題で効くのは「**左下**に点があるか」だけ、と覚えておく。
